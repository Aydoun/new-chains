...
export function CarouselFrame({ asChild, className, children }: CarouselFrameProps) {
...
export function Carousel({ frames, initialIndex = 0, className }: CarouselProps) {
  ...
  React.useEffect(() => {
    ...
    setCurrentIndex((previous) => ((previous % frameCount) + frameCount) % frameCount);
  }, [frameCount]);
    ...
    <div className={cn("flex flex-col gap-4", className)}>
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <span className="text-sm text-muted-foreground">
          Frame {currentIndex + 1} of {frameCount}
        </span>
